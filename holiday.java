...
public class Holiday {
  ...
  static public void main(String[] args) {
    ...
    int weekday = DayOfWeek(9, 1, year);   // Day of week for 9/1.
  ...
  static boolean ValidDate (int month, int day, int year) {
    ...
    if (year >= 1900 && year <= 2999 && month >= 1 && month <= 12) {
      ...
      days = daysInMonth[month-1]; // (-1, because the indices of an array start from 0.)
   ...
   static long DaysSince1900 (int month, int day, int year) {
     ...
     // Calculate days to 01-01 of this year with correction for all the previous leap years:
     if (year == 1900)
       days = 0;
     else {
	...
	// Add days for previous months with correction for the current leap year:
